...
/**
 * Outbox 中保存的 reply 结构。
 *
 * 可以理解为：reply + 重试状态。
 */
export type SpringImOutboxEntry = SpringImReply & {
  attempts: number;
  nextAttemptAt: number;
  lastError?: string;
  status?: "queued" | "sending" | "retry" | "dead";
  priority?: number;
};

export type SpringImDeadLetterEntry = SpringImOutboxEntry & {
  deadAt: number;
  reason: string;
};

export type SpringImStoredMessage = SpringImCloudMessage & {
  receivedAt: number;
  dispatchStatus: "queued" | "processing" | "processed" | "failed" | "duplicate";
  lastError?: string;
};
...
export type SpringImState = {
  ...
  messages?: Record<string, SpringImStoredMessage>;
  outbox?: SpringImOutboxEntry[];
  deadLetters?: SpringImDeadLetterEntry[];
  runtime?: {
...
/**
 * `/events/replay` 的 HTTP 响应结构（兼容路径）。
 *
 * 新主链不再主动依赖它；这里只保留给旧客户端和排障。
 */
export type SpringImReplayResponse = {
  events: SpringImCloudMessage[];
  hasMore?: boolean;
};
