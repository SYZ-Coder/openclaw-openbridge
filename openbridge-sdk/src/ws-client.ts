...
import { signHello, type SpringImDeviceIdentity } from "./state.js";
...
export class SpringImWebSocketClient {
  ...
  async connect(params: {
    ...
    const helloFrame: ClientFrame = {
      ...
      nonce,
      signature,
      lastProcessedSequence: params.resumeMetadata?.lastProcessedSequence,
      lastProcessedEventId: params.resumeMetadata?.lastProcessedEventId,
    };
    this.send(helloFrame);
    this.logger.info(
      `openbridge[${this.account.accountId}]: websocket hello sent deviceId=${params.identity.deviceId} lastProcessedSequence=${params.resumeMetadata?.lastProcessedSequence ?? "n/a"}`,
    );
