 ...
 * 然后调用共享的 OpenClaw reply pipeline，并把最终 reply 通过 WebSocket 回传云服务。
 */
...
type RuntimeChannel = {
  routing: {
    resolveAgentRoute: (params: Record<string, unknown>) => { agentId?: string; sessionKey?: string };
  };
  session: {
    resolveStorePath: (store: unknown, params: Record<string, unknown>) => string;
...
export async function dispatchCloudMessage(params: {
  ...
  params.logger.info(
    `openbridge[${params.account.accountId}]: dispatch route resolved ${summarizeCloudMessage(params.message)} agentId=${route.agentId ?? "(missing)"} sessionKey=${route.sessionKey ?? "(missing)"} storeAgentId=${route.agentId ?? "(missing)"}`,
  );
  if (!route.agentId || !route.sessionKey) {
    params.logger.warn(
      `openbridge[${params.account.accountId}]: route is incomplete eventId=${params.message.eventId} agentId=${route.agentId ?? "(missing)"} sessionKey=${route.sessionKey ?? "(missing)"}`,
    );
  }
  ...
  const ctxPayload = runtime.reply.finalizeInboundContext?.(inboundPayload) ?? inboundPayload;
  ...
  params.logger.info(
    `openbridge[${params.account.accountId}]: recordInboundSession start eventId=${params.message.eventId} sessionKey=${ctxPayload.SessionKey ?? route.sessionKey ?? "(missing)"} storePath=${storePath} elapsedMs=${Date.now() - dispatchStartedAt}`,
  );
  ...
  params.logger.info(
    `openbridge[${params.account.accountId}]: recordInboundSession done eventId=${params.message.eventId} elapsedMs=${Date.now() - dispatchStartedAt}`,
  );

  params.logger.info(
    `openbridge[${params.account.accountId}]: reply dispatcher start eventId=${params.message.eventId} sessionKey=${ctxPayload.SessionKey ?? route.sessionKey ?? "(missing)"} elapsedMs=${Date.now() - dispatchStartedAt}`,
  );
  await runtime.reply.dispatchReplyWithBufferedBlockDispatcher({
    ...
    dispatcherOptions: {
      ...
      onError: (err: unknown, info: { kind?: string }) => {
        params.logger.error(
          `openbridge[${params.account.accountId}]: dispatch ${info.kind ?? "reply"} failed eventId=${params.message.eventId}: ${String(err)}`,
        );
      },
    },
    replyOptions: {
  ...
  params.logger.info(
    `openbridge[${params.account.accountId}]: reply dispatcher done eventId=${params.message.eventId} elapsedMs=${Date.now() - dispatchStartedAt}`,
  );
}
